...
class Crawler:
			def __init__(self):
					...
					self.stack_test=[]
					self.block_terms_url=['javascript:','#','javascript:;',None,'/','','javascript:void();']
					self.base_url=''
					self.basenew_url=''
					self.crawl_count=50
					self.count_main=0
					self.exclude=('.mp3','.png','.jpg','.gif','.txt','.pdf','.jpeg','.flv','.mp4','.zip','.css','.js')
			...
			def crawl_start(self,url,count):
				...
				try:
					data=requests.get(url,allow_redirects=True,timeout=2.0).content
				except Exception:
					sys.exc_clear()
			...
			def crawl_run(self):
				...
				if len(list(set(self.stack_main))) < self.crawl_count and len(self.stack_main) > self.count_main:
					self.stack_to_be_indexed=[]
					self.stack_to_be_indexed=self.pull_links(requests.get(self.stack_main[self.count_main],allow_redirects=True,timeout=2.0).content)
					self.count_main+=1
					self.crawl_run()
			...
			def pull_links(self,data):
				...
				for links in data.find_all('a'):
					if links.get('href') not in self.block_terms_url:
						if links.get('href').startswith(self.base_url) or links.get('href').startswith('http'):
							link=str(links.get('href'))
						elif links.get('href').startswith('/'):
							link=str(self.base_url)+str(links.get('href')[1:])
						else:
							link=str(self.base_url)+str(links.get('href'))
						if link not in self.stack_main and not link.endswith(self.exclude):
...
class Social:
		...
		def social_data(self,url):
			...
			social_urls=[
			"https://graph.facebook.com/fql?q=SELECT%20url,%20share_count,%20like_count,%20comment_count,%20total_count,normalized_url,commentsbox_count,%20click_count%20FROM%20link_stat%20WHERE%20url='"+str(urllib2.quote(url))+"'",
			"http://urls.api.twitter.com/1/urls/count.json?url="+str(urllib2.quote(url))+"&callback=",
			"http://www.linkedin.com/countserv/count/share?url="+str(urllib2.quote(url))+"&format=json",
			"https://plusone.google.com/_/+1/fastbutton?url="+str(urllib2.quote(url))
			]
			mainurl={"url":url}

			# Pinterest sometime block continous requests to its server
			pinsurl="http://widgets.pinterest.com/v1/urls/count.json?source=6&url="+str(urllib2.quote(url))
			...
			for response in grequests.map(rs):
				...
				if count == 4:
					...
					return dict(mainurl.items() + fblike.items() + fbshare.items() + fbcomment.items() + twitter.items() + linkedin.items() + pins.items() + plusone.items())